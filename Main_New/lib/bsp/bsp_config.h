...
typedef struct __attribute__((packed))
{
    ...
    type_mtfc_phase_gate_config_t phase_pin_config[MAX_SIDE];//du lieu cai dat tung pin tin hieu den
...
typedef struct __attribute__((packed))
{
    ...
    type_one_side_lamp_time_t side[MAX_SIDE];   //thoi gian xanh vang do dang thuc thi
    ...
    type_time_walking_green_flashing_t t_flashing_wg;//Thoi gian bat dau chop tin hieu den vàng
    uint8_t railway_connect_phase_config[MAX_SIDE];//Pha nao co ket noi duong sat thi = 1 nguoc lai = 0
    uint8_t current_phase_is_green;             //pha nao dang o den xanh =0 nghia la dang chop vang hoac off
    ...
    uint8_t index_side_manual_select;           //pha tin hieu duoc chon trong che do manual
    volatile uint8_t mtfc_main_state;           //Trang thai dang khoi dong hay vao che do hd chinh
...
typedef struct __attribute__((packed))
{
    ...
    volatile bool is_ticker_one_seconds_seek;    //dùng cho tiến trình đếm hệ thống đèn
    volatile bool is_ticker_one_seconds_gateway; //dùng cho xử lý tiến trình 1s gateway
    volatile bool is_ticker_one_seconds_main;    //dùng xử lý tiến trình 1s hàm main
    volatile bool is_ticker_one_seconds_glcd;    //Dung xu ly cac tien trinh 1s cua glcd

    volatile bool is_receive_card_config_feedback; //dữ liệu cài đặt phản hồi từ card
    ...
    volatile bool is_update_schedule_to_gateway;        //truyền dữ liêu schedule cho gateway
    volatile bool is_update_mtfc_config_to_gateway;     //Truyền dữ liệu cài đặt thiết bị đến gateway
    volatile bool is_update_mtfc_coordinate_to_gateway; //Truyền tọa độ của thiết bị
    volatile bool is_update_mtfc_details_to_gateway;    //truyền thông tin cơ bản của thiết bị
    ...
    volatile bool is_update_schedule_to_cloud;          //update thoi gian hoat dong len gateway
    ...
    volatile bool is_card_config_request;         //Yeu cau lay thong tin card tu phan cung cua mtfc
    ...
    volatile bool is_time_location_load;    //Yeu cau cap nhat thoi gian len man hinha
    ...
    volatile bool is_import_sd_feedback; //phan hoi trang thai import tu module ex card
    volatile bool is_export_sd_feedback; //phan hoi trang thai export tu module ex card
    ...
    volatile bool is_server_trasfer_one_cycle_form;//Nhan 1 cycle form tu phia server
    volatile bool is_save_schedule_form_server;//Luu chien lược hoạt động từ phía server
