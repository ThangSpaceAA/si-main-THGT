...
void mtfc_watdog_init(float time_wd)
{
  ...
  else if (time_wd_reload > 4095)
  {
    time_wd_reload = 4095;
  }
  debug(USING_WATCHDOG_TIMER, "\r\nmtfc watchdog init reload: %d", time_wd_reload);
  iwdg_init(IWDG_PRE_256, time_wd_reload);
}
...
void mtfc_memory_startup_config(void)
{
  ...
  // Never let corrupted/stale flash data drive array indexes or output loops.
  if ((mtfc_schedule.num_side < 2) || (mtfc_schedule.num_side > MAX_SIDE))
  {
    debug(MAIN_DEBUG, "%s", "\r\nInvalid schedule in flash. Loading safe defaults");
    mtfc_contruct_schedule_default(&mtfc_schedule, DEFAULT_NUM_PHASES);
    schedule_update_from_screen = mtfc_schedule;
    mtfc_mem.mem_save_data(BLOCK_2, (char *)&mtfc_schedule, sizeof(type_mtfc_schedule_t));
  }
...
int main()
{
  ...
  // Main CPU owns the controller lifecycle. CMD_CPU_TO_MASTER_START from an
  // older CM4 remains accepted for compatibility, but it no longer gates boot.
...
#if USING_WATCHDOG_TIMER == 1
  // Start IWDG after the intentionally blocking boot/card-detect sequence.
  mtfc_watdog_init(3.0);
#endif
  while (true)
  {
    // UART callbacks are dispatched in main context, never in the RX ISR.
    check_card_com.process_rx();
    rf_com.process_rx();
    main_rs232_com.process_rx();
...
void mtfc_check_phase_connect_railway(type_mtfc_cycle_working_package_t *mtfc_wk, type_mtfc_config_t *mtfc_config, type_mtfc_schedule_t *mtfc_schedule)
{
  ...
  for (uint8_t j = 0; j < MAX_SIDE; j++)
  {
    ...
    if ((mtfc_config->index_card[j].is_railway_enabled == 1) &&
        (mtfc_config->index_card[j].phase >= 1) &&
        (mtfc_config->index_card[j].phase <= MAX_SIDE))
    {
...
uint8_t mtfc_card_detect(type_mtfc_cycle_working_package_t *obj)
{
  ...
  {
    // Card detection runs before the main loop, so service deferred UART
    // frames here as well.
    check_card_com.process_rx();
    rf_com.process_rx();
    main_rs232_com.process_rx();
...
static bool mtfc_ex_card_payload_is_valid(uint8_t cmd, uint8_t size)
{
  ...
  case CMD_CPU_TO_MASTER_TIME_ACTIVE:
    return size >= sizeof(type_active_lamp_t);
  case CMD_CPU_TO_MASTER_CYCLE_FORM:
    return size >= sizeof(type_mtfc_quick_cycle_config_t);
  case CMD_CPU_TO_MASTER_SET_RTC_MANUAL:
    return size >= sizeof(type_date_time_t);
  case CMD_CPU_TO_MASTER_CHANGE_PHASE_1:
    return size >= sizeof(type_mtfc_one_card_config_t);
  case CMD_CPU_TO_MASTER_SCHEDULE_TIME_UPDATE:
    return size >= sizeof(type_time_lamp_form_t);
  case MSP_APP_CYCLE_FORM:
    return size >= sizeof(cycle_t);
  case CMD_CPU_TO_MASTER_SCHEDULE_DAYS_UPDATE:
    return size >= sizeof(type_days_lamp_form_t);
  case CMD_CPU_TO_MASTER_TIME_SETTING:
    return size >= sizeof(time_setting_t);
  default:
    return true;
  }
}
...
{
  uint8_t cmd = main_rs232_com.get_cmd();
  debug(MAIN_DEBUG, "\r\nmain_rs232_com.get_cmd(): [%d]", cmd);
  if (!mtfc_ex_card_payload_is_valid(cmd, siz))
  {
    debug(MAIN_DEBUG, "\r\nDrop RS232 cmd [%d], invalid payload size [%d]", cmd, siz);
    return;
  }
  ...
  case CMD_CPU_TO_MASTER_SOPHA:
  {
    uint8_t requested_num_side = main_rs232_com.read8();
    if ((requested_num_side < 2) || (requested_num_side > MAX_SIDE))
    {
      debug(MAIN_DEBUG, "\r\nReject invalid num side: [%d]", requested_num_side);
      break;
    }
    debug(MAIN_DEBUG, "\r\n%d", schedule_update_from_screen.num_side);
    schedule_update_from_screen.num_side = requested_num_side;
...
void mtfc_output_signal_data_to_card(type_mtfc_cycle_working_package_t *mtfc, uint8_t *signal, uint8_t num_side)
{
  ...
  // The physical card output is the critical path. Optional telemetry ports
  // are updated only after the controller card has received the new output.
...
int8_t mtfc_check_imei(uint8_t idx, type_mtfc_cycle_working_package_t *mtfc_wk, type_mtfc_config_t *mtfc_config)
{
  ...
  if ((mtfc_config->index_card[idx].imei == 0) ||
      (mtfc_config->index_card[idx].phase == 0) ||
      (mtfc_config->index_card[idx].phase > MAX_SIDE))
  {
    return -1;
  }
  return 0;
...
void mtfc_sensor_detect_processing(type_mtfc_cycle_working_package_t *mtfc_wk, type_mtfc_config_t *mtfc_config)
{
  ...
  if (millis() - mtfc_get_card_sensor_timeline >= 1500)
  {
    ...
    bool valid_card_found = false;
    for (uint8_t checked = 0; checked < MAX_SIDE; checked++)
    {
      if (mtfc_check_imei(mtfc_wk->idx_check_sensor, mtfc_wk, mtfc_config) == 0)
      {
        valid_card_found = true;
        break;
      }
      (*mtfc_wk).idx_check_sensor++;
      if (mtfc_wk->idx_check_sensor >= MAX_SIDE)
      {
